...
### 4.6 "逻辑正确但语义错误"的语义级 bug

**现象**：`drawImpl` 中光环分层排序时，他人的光环问卷被错误地提到了光环层，排在了用户自己的普通问卷前面。但功能运行正常，不报错、不抛异常。

**根因**：代码在分层过滤时写了 `p.haloActive`（所有光环问卷）而非 `p.haloActive && p.userId === uid`（仅自己的光环问卷）。光环是视角诡计的一部分——它应该只对自己生效，他人的光环问卷在当前用户眼里应该归入"他人层"。写成过滤所有光环问卷是"语法正确、语义错误"的典型。
...
**为什么测试没发现**：三层测试体系全放过了：
- 纯函数测试（test-stage6.mjs）—— 验证的是 `calculateWeight` 返回值，不涉及分层排序
- Mock 页在线测试（pin-test.vue）—— 验证的是 draw 返回数据结构 + 数量，不校验"光环问卷的 userId 是否等于当前用户"
- 验收清单 —— 没有列"他人的问卷不应该有光环视觉效果"这条

**教训**：有些 bug 既不是语法错误也不是逻辑错误，而是 **"语义映射错误"**——代码在机械层面正确（执行了"选择所有光环问卷"），但代码与设计意图的对应关系错了（设计意图是"选择自己的光环问卷"）。三层测试体系都覆盖不到语义映射，因为测试验证的是"执行结果"，而语义映射错误产生的执行结果看起来依然"正确"——只是在业务视角下效果不对。

**如何预防**：这类 bug 只有通过**代码审阅 + 与设计文档逐句比对**才能发现。具体做法：
1. 把设计文档相关段落摘出来（"光环只对自己的问卷生效"）
2. 逐行比对代码实现（"这一行的过滤条件是否和设计文档一致"）
3. 在验收清单中增加**语义级校验项**，比如"他人的光环问卷在抽取结果中不应标记为光环"

这一点纯函数测不了、Mock 页测不了——只能人审。
